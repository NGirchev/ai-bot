 ...
 */
@Slf4j
@SpringBootTest(classes = SpringAIOllamaDnsIntegrationTest.TestConfig.class)
@ComponentScan(
...
@TestPropertySource(properties = {
    ...
    "ai-bot.common.openrouter-free-models.enabled=false",
    // Исключаем автоконфигурации, которые не нужны для теста
    "spring.autoconfigure.exclude=org.springframework.boot.autoconfigure.jdbc.DataSourceAutoConfiguration,org.springframework.boot.autoconfigure.orm.jpa.HibernateJpaAutoConfiguration,org.springframework.boot.autoconfigure.orm.jpa.JpaRepositoriesAutoConfiguration,org.springframework.boot.autoconfigure.flyway.FlywayAutoConfiguration,org.springframework.ai.model.openai.autoconfigure.OpenAiAutoConfiguration,org.springframework.ai.model.openai.autoconfigure.OpenAiChatAutoConfiguration,org.springframework.ai.model.openai.autoconfigure.OpenAiAudioSpeechAutoConfiguration,org.springframework.ai.model.openai.autoconfigure.OpenAiAudioTranscriptionAutoConfiguration,org.springframework.ai.model.openai.autoconfigure.OpenAiEmbeddingAutoConfiguration,org.springframework.ai.model.openai.autoconfigure.OpenAiImageAutoConfiguration,org.springframework.ai.model.openai.autoconfigure.OpenAiModerationAutoConfiguration,org.springframework.ai.model.chat.memory.autoconfigure.ChatMemoryAutoConfiguration,org.springframework.ai.model.chat.memory.repository.jdbc.autoconfigure.JdbcChatMemoryRepositoryAutoConfiguration,ru.girchev.aibot.common.config.CoreAutoConfig,ru.girchev.aibot.ai.springai.config.SpringAIAutoConfig"
})
class SpringAIOllamaDnsIntegrationTest {
...
//                .user("Напиши сказку").call().chatResponse().getResult().getOutput().getText());
        ...
        var responseFlux = ChatClient.builder(ollamaChatModel).build().prompt()
                ...
                .stream()
                .chatResponse();
        ChatResponse chatResponse = AIUtils.processStreamingResponseByParagraphs(responseFlux, text -> {
            try {
                System.out.write(text.getBytes(StandardCharsets.UTF_8));
            } catch (IOException e) {
                throw new RuntimeException(e);
