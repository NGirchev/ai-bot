...
public class OpenRouterFreeModelResolver {
    ...
    private final OpenRouterFreeModelsProperties openRouterFreeModelsProperties;

    private volatile List<String> cachedFreeOpenRouterModels;
    private volatile long lastRefreshAtEpochMs;
     ...
     */
    public List<String> candidatesForModel(String requestedModelId, Set<ModelType> requiredModelTypes) {
        if (!StringUtils.hasText(requestedModelId)) {
            return Collections.emptyList();
        }
        if (OPENROUTER_AUTO.equals(requestedModelId)) {
            return candidatesForAutoRequest(requiredModelTypes);
        }
        if (requestedModelId.contains(FREE_SUFFIX)) {
            List<String> candidates = candidatesForAutoRequest(requiredModelTypes);
     ...
     * Сейчас реально учитывается только {@link ModelType#TOOL_CALLING}.
     */
    public List<String> candidatesForAutoRequest(Set<ModelType> requiredModelTypes) {
        ...
        OpenRouterFreeModelsProperties.Ranking ranking = openRouterFreeModelsProperties.getRanking();
        boolean rankingEnabled = Boolean.TRUE.equals(ranking != null ? ranking.getEnabled() : null);

        List<String> eligible = applyCapabilityFilters(filteredByProps, requiredModelTypes);
        if (eligible.isEmpty() && requiredModelTypes != null && !requiredModelTypes.isEmpty()) {
    ...
    private String selectBestFreeOpenRouterModelId() {
        try {
            ...
            if (cached == null) {
                ...
                cachedFreeOpenRouterModels = cached;
                lastRefreshAtEpochMs = System.currentTimeMillis();
            }
            ...
            OpenRouterFreeModelsProperties.Ranking ranking = openRouterFreeModelsProperties.getRanking();
            boolean rankingEnabled = Boolean.TRUE.equals(openRouterFreeModelsProperties.getEnabled())
                    && ranking != null
    ...
    private Comparator<String> candidateComparator(long nowEpochMs) {
        return Comparator.<String>comparingDouble(id -> score(id, nowEpochMs)).reversed()
                ...
                .thenComparing((String id) -> infoByModelId.getOrDefault(id, ModelInfo.UNKNOWN).toolsSupported, Comparator.reverseOrder())
    ...
    private List<String> loadFreeOpenRouterModelIds() {
        OpenRouterFreeModelsProperties.Api api = openRouterFreeModelsProperties.getApi();
        ...
        log.info("OpenRouter free-model discovery: loaded {} free models", free.size());
        log.info("OpenRouter free-model discovery: free models list:\n- {}", String.join("\n- ", free));
        return free;
    }
    ...
    private List<String> applyCapabilityFilters(List<String> modelIds, Set<ModelType> requiredModelTypes) {
        if (modelIds == null || modelIds.isEmpty()) {
            return Collections.emptyList();
        }
        if (requiredModelTypes == null || requiredModelTypes.isEmpty()) {
            return modelIds;
        }
        if (!requiredModelTypes.contains(ModelType.TOOL_CALLING)) {
            return modelIds;
        }
        return modelIds.stream()
                .filter(id -> infoByModelId.getOrDefault(id, ModelInfo.UNKNOWN).toolsSupported)
                .toList();
    }
    ...
    private List<String> applyPropertyFilters(List<String> modelIds) {
        ...
        OpenRouterFreeModelsProperties.Filters filters = openRouterFreeModelsProperties.getFilters();
        if (filters == null) {
            return modelIds;
        }
    ...
    private ModelInfo extractModelInfo(JsonNode modelNode) {
        ...
        if (!toolsSupported && capabilities.isObject()) {
            JsonNode tools = capabilities.get("tools");
            if (tools != null) {
                toolsSupported = tools.asBoolean(false);
            }
        }

        return new ModelInfo(toolsSupported);
    }
    ...
    private record ModelInfo(boolean toolsSupported) {
        private static final ModelInfo UNKNOWN = new ModelInfo(false);
    }
    ...
    private void logRankingDecision(List<String> selectedCandidates, long nowEpochMs) {
        ...
        for (String modelId : selectedCandidates) {
            ...
            sb.append(" tools=").append(info.toolsSupported);
            if (stats != null) {
